...
class Conferencia extends \Core\Model
{
    ...
    {    
        ...
        {
            $db = static::getDB();
            $stmt = $db->query("SELECT conferencias.id, conferencias.conferencia, conferencias.fecha, conferencias.horario, conferencias.duracion, conferencias.activo, usuarios.usuario, conferencias.id_usuario FROM `conferencias` INNER JOIN usuarios ON conferencias.id_usuario = usuarios.id WHERE conferencias.id=$id");
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            return $results;
        } catch (PDOException $e) {
            echo $e->getMessage();
